...
$eligibility = $conn-> prepare("SELECT sentence FROM `user_test_appear` WHERE `email` = ? ");
...
if(isset($_POST['startTest'])){
    $newValue = (int)$eligible['sentence'] + 1;

    $stmt3 = $conn->prepare("UPDATE `user_test_appear` SET `sentence` = ? WHERE `email` = ?");
...
<body>
    <div class="header-part">
     ...
     <div class="main-part">
        <h1>Completing Sentences : English & Bangla</h1>
        <form action="" method="post">
        <?php if($_SESSION['user_type'] == 'regular') {
                if($eligible['sentence'] < 2){ ?>
     ...
     <div class="instruction">
        <h2>Instructions :</h2>
        <ul>
            <li>First 25 will be english sentences</li>
            <li>Second 25 will be bangla sentences</li>
            <li>For each segment you'll get 5 minutes</li>
